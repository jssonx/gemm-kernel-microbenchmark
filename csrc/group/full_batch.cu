...
template <typename Gemm>
Result TestbedBatched<Gemm>::profile_batched_gemm(
    cutlass::Distribution::Kind init_A,
    cutlass::Distribution::Kind init_B,
    cutlass::Distribution::Kind init_C)
{
    this->initialize_tensor(this->block_A.get(), this->block_A.size(), init_A, this->seed * 2021);
    this->initialize_tensor(this->block_B.get(), this->block_B.size(), init_B, this->seed * 2022);
    this->initialize_tensor(this->block_C.get(), this->block_C.size(), init_C, this->seed * 2023);

    cutlass::reference::device::BlockFillSequential(
        this->block_D.get(), this->block_D.size(), typename Gemm::ElementC(), typename Gemm::ElementC());

    Result result = this->profile();
    return result;
}
...
int main(int argc, char const **args)
{
    ...
    // Profile the first batch with random initialization
    Result result1 = testbed_batched.profile_batched_gemm(
    ...
    // Profile the second batch with A from the first batch's result (C or D)
    Result result2 = testbed_batched.profile_batched_gemm(
